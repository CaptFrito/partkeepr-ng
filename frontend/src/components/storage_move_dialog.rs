...
use crate::api;
use crate::{DataVersion, StorageMoveCtx, StorageMoveState};

/// Combined move-to picker for storage entities. The single dialog
/// handles both folder-move (with subtree pruning) and location-move
/// (no pruning needed since leaves can sit under any folder). Picker
/// data comes from the storage-categories flat list — locations don't
/// appear there because they're not valid drop targets, only folders
/// are.
#[component]
pub fn StorageMoveDialog() -> impl IntoView {
    let dialog_state = expect_context::<StorageMoveState>().0;
    let data_version = expect_context::<DataVersion>().0;

    let categories =
        LocalResource::new(|| async { api::fetch_storage_cats_flat_full().await });
    ...
    let submit = move || {
        ...
        spawn_local(async move {
            let res = match ctx {
                StorageMoveCtx::MoveCat { id, .. } => {
                    api::move_storage_cat(id, parent).await
                }
                StorageMoveCtx::MoveLoc { id, .. } => {
                    api::move_storage_loc(id, parent).await
                }
            };
            match res {
                Ok(()) => {
    ...
    let title = move || dialog_state.get().map(|c| match c {
        StorageMoveCtx::MoveCat { name, .. } => format!("Move folder \"{name}\"…"),
        StorageMoveCtx::MoveLoc { name, .. } => format!("Move location \"{name}\"…"),
    }).unwrap_or_default();
                        ...
                        <Suspense fallback=|| view! { <p class="muted">"Loading…"</p> }>
                            {move || {
                                ...
                                categories.get().map(|res| match (&*res, ctx) {
                                    (Ok(rows), Some(StorageMoveCtx::MoveCat { id, .. })) => {
                                        // Prune the moved folder's own subtree.
                                    ...
                                    (Ok(rows), Some(StorageMoveCtx::MoveLoc { .. })) => {
                                        // No pruning for leaf moves.
                                        view! {
                                            <PickerTree rows=rows.clone() chosen=chosen_parent/>
                                        }.into_any()
                                    }
                                    (Err(e), _) => view! {
